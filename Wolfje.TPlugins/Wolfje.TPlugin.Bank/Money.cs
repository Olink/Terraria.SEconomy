...
namespace Wolfje.Plugins.SEconomy {
    /// <summary>
    /// A representation of Money in Seconomy.  Money objects are toll-free bridged with integers (int).
    /// </summary>
    public struct Money {
        //ye olde godly value type
        ...
        public static Money Parse(string MoneyRepresentation) {
            Regex moneyRegex = new Regex(@"(-)?((\d*)p)?((\d*)g)?((\d*)s)?((\d*)c)?", RegexOptions.IgnoreCase);
            long totalMoney = 0;
